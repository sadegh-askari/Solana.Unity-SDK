    ...
    public class SolanaWalletAdapterWebGL : WalletBase
    {
        ...
        private static async Task SetCurrentWallet()
        {
            ...
            if (_currentWallet == null)
            {
                ...
                {
                    Debug.LogError("Find Wallet: " + wallet.name + " " + wallet.installed);
                }
                _currentWallet = Array.Find(Wallets,
                    wallet => wallet.name.Equals(_walletOptions.walletName,
        ...
        [DllImport("__Internal")]
        private static extern void ExternSignTransactionWallet(string walletName, string transaction,
            Action<string> callback);

        [DllImport("__Internal")]
        private static extern void ExternSignAllTransactionsWallet(string walletName, string transactions,
            Action<string> callback);

        [DllImport("__Internal")]
        private static extern void ExternSignMessageWallet(string walletName, string messageBase64,
            Action<string> callback);
